...
var serializer_default = {
  ...
  */
  ...
  binaryEncode(message2) {
    let { join_ref, ref: ref2, event, topic, payload } = message2;
  ...
  */
  ...
  decodeReply(buffer, view, decoder2) {
    ...
    let joinRef = decoder2.decode(buffer.slice(offset, offset + joinRefSize));
    offset = offset + joinRefSize;
    let ref2 = decoder2.decode(buffer.slice(offset, offset + refSize));
...
function getSupabase(env2) {
  return createClient(env2.SUPABASE_URL, env2.SUPABASE_SERVICE_KEY, {
...
var JWKS_URL = "https://axjzakmelmoqmzuxvwsa.supabase.co/auth/v1/.well-known/jwks.json";
var authMiddleware = createMiddleware(async (c, next) => {
  ...
  try {
    ...
    const { data: user, error: error3 } = await supabase.from("users").select("*").eq("id", userId).eq("is_active", true).single();
    if (error3 || !user) return c.json({ error: "Not authenticated" }, 401);
    c.set("userId", userId);
    c.set("user", user);
  } catch {
    return c.json({ error: "Not authenticated" }, 401);
...
// .wrangler/tmp/bundle-OdzxUW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-OdzxUW/middleware-loader.entry.ts
