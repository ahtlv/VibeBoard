...
class BoardLimitExceededError(Exception):
    pass
...
class BoardService:
    ...
    async def create(self, data: BoardCreate, current_user: User) -> Board:
        ...
        member = await self._ws_repo.get_member(workspace_id, current_user.id)
        if member is None:
            raise WorkspaceAccessDeniedError(workspace_id)

        if current_user.plan == 'free':
            count = await self._board_repo.count_for_user(current_user.id)
            if count >= FREE_BOARD_LIMIT:
                raise BoardLimitExceededError(count)
    ...
    async def list_for_workspace(
        ...
        *,
        ...
        return await self._board_repo.list_for_workspace(
